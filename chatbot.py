...
def model_inputs():
    inputs = ops.placeholder(ops.int32, [None, None], name='input')
    targets = ops.placeholder(ops.int32, [None, None], name='target')
    lr = ops.placeholder(ops.float32, name='learning_rate')
    keep_prob = ops.placeholder(ops.float32, name='keep_prob')
    return inputs, targets, lr, keep_prob

# Preprocessing the targets
def preprocess_targets(targets, word2int, batch_size):
    left_side = ops.fill([batch_size, 1], word2int['<SOS>'])
    right_side = ops.strided_slice(targets, [0, 0], [batch_size, -1], [1, 1]) # batch_size being the number of lines
    preprocessed_targets = ops.concat([left_side, right_side], 1) # 1 = horizontal; 2 = vertical
    return preprocessed_targets

# Creating the Encoder RNN Layer
def encoder_rnn(rnn_inputs, rnn_size, num_layers, keep_prob, sequence_length):
    lstm = ops.contrib.rnn.BasicLSTMCell(rnn_size)
    lstm_dropout = ops.contrib.rnn.DropoutWrapper(lstm, input_keep_prob=keep_prob) # Deactivates neurons during training
    encoder_cell = ops.contrib.rnn.MultiRNNCell([lstm_dropout] * num_layers)
    # Outputs - encoder_output, encoder_state; we only need the encoder_state
    _, encoder_state = ops.nn.bidirectional_dynamic_rnn(cell_fw=encoder_cell, 
...
def decode_training_set(encoder_state, decoder_cell, decoder_embedded_input, sequence_length, decoding_scope, output_function, keep_prob, batch_size):
    attention_states = ops.zeros([batch_size, 1, decoder_cell.output_size])
    attention_keys, attention_values, attention_score_function, attention_construct_function = ops.contrib.seq2seq.prepare_attention(attention_states, attention_option='bahdanau', num_units=decoder_cell.output_size)
    training_decoder_function = ops.contrib.seq2seq.attention_decoder_fn_train(encoder_state[0],
                                                                              attention_keys,
                                                                              attention_values,
    ...
    decoder_output, _, _ = ops.contrib.seq2seq.dynamic_rnn_decoder(decoder_cell,
                                                                  training_decoder_function,
                                                                  decoder_embedded_input,
                                                                  sequence_length,
                                                                  scope=decoding_scope)
    decoder_output_dropout = ops.nn.dropout(decoder_output, keep_prob)
...
def decode_test_set(encoder_state, decoder_cell, decoder_embeddings_matrix, sos_id, eos_id, maximum_length, num_words, decoding_scope, output_function, keep_prob, batch_size):
    attention_states = ops.zeros([batch_size, 1, decoder_cell.output_size])
    attention_keys, attention_values, attention_score_function, attention_construct_function = ops.contrib.seq2seq.prepare_attention(attention_states, attention_option='bahdanau', num_units=decoder_cell.output_size)
    test_decoder_function = ops.contrib.seq2seq.attention_decoder_fn_inference(output_function,
                                                                              encoder_state[0],
                                                                              attention_keys,
                                                                              attention_values,
                                                                              attention_score_function,
    ...
    test_predictions, _, _ = ops.contrib.seq2seq.dynamic_rnn_decoder(decoder_cell,
...
def decoder_rnn(decoder_embedded_input, decoder_embeddings_matrix, encoder_state, num_words, sequence_length, rnn_size, num_layers, word2int, keep_prob, batch_size):
    with ops.variable_scope('decoding') as decoding_scope:
        lstm = ops.contrib.rnn.BasicLSTMCell(rnn_size)
        lstm_dropout = ops.contrib.rnn.DropoutWrapper(lstm, input_keep_prob=keep_prob) # Deactivates neurons during training
        decoder_cell = ops.contrib.rnn.MultiRNNCell([lstm_dropout] * num_layers)
        weights = ops.truncated_normal_initializer(stddev=0.1)
        biases = ops.zeros_initializer()
        output_function = lambda x: ops.contrib.layers.fully_connected(x, num_words, None, scope=decoding_scope, weights_initializer=weights, biases_initializer=biases)
...
def seq2seq_model(inputs, targets, keep_prob, batch_size, sequence_length, answers_num_words, questions_num_words, encoder_embedding_size, decoder_embedding_size, rnn_size, num_layers, questionswords2int):
    encoder_embedded_input = ops.contrib.layers.embed_sequence(inputs, answers_num_words + 1, encoder_embedding_size, initializer=ops.random_uniform_initializer(0, 1))
    encoder_state = encoder_rnn(encoder_embedded_input, rnn_size, num_layers, keep_prob, sequence_length)
    preprocessed_targets = preprocess_targets(targets, questionswords2int, batch_size)
    decoder_embeddings_matrix = ops.Variable(ops.random_uniform([questions_num_words + 1, decoder_embedding_size], 0, 1))
    decoder_embedded_input = ops.nn.embedding_lookup(decoder_embeddings_matrix, preprocessed_targets)
...
sequence_length = ops.placeholder_with_default(25, None, name='sequence_length')

# Getting the shape of the inputs tensor
input_shape = ops.shape(inputs)

# Getting the training and test predictions
training_predictions, test_predictions = seq2seq_model(ops.reverse(inputs, [-1]), 
                                                       targets, 
                                                       keep_prob, 
                                                       batch_size, 
                                                       sequence_length, 
...
with ops.name_scope('optimization'):
    loss_error = ops.contrib.seq2seq.sequence_loss(training_predictions,
                                                  targets,
                                                  ops.ones([input_shape[0], sequence_length]))
    optimizer = ops.train.AdamOptimizer(learning_rate)
    gradients = optimizer.compute_gradients(loss_error)
    clipped_gradients = [(ops.clip_by_value(grad_tensor, -5., 5.), grad_variable) for grad_tensor, grad_variable in gradients if grad_tensor is not None]
...
session.run(ops.global_variables_initializer())
...
checkpoint = './chatbot_weights.ckpt'
session = ops.InteractiveSession()
session.run(ops.global_variables_initializer())
saver = ops.train.Saver()
